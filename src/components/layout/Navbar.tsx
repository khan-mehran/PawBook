...
const Navbar: React.FC = () => {
  ...
  return (
    <nav
      className="fixed top-0 left-0 right-0 z-50 backdrop-blur-xl border-b border-[var(--pb-border-faint)] transition-colors duration-300"
      style={{ backgroundColor: 'var(--pb-nav-bg)' }}
    >
      <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            className="md:hidden p-2 rounded-full text-[var(--pb-muted)] hover:text-[var(--pb-text)] hover:bg-[var(--pb-hover)] transition-colors cursor-pointer"
          ...
          <div className="relative">
            <motion.button
              ...
              className="relative p-2 rounded-full text-[var(--pb-muted)] hover:text-[var(--pb-text)] hover:bg-[var(--pb-hover)] transition-colors cursor-pointer"
            ...
            >
              ...
              {unreadCount > 0 && (
                <motion.span
                  ...
                  initial={{ scale: 0.5 }}
                  animate={{ scale: 1 }}
                  className="absolute top-0.5 right-0.5 min-w-[1.1rem] h-[1.1rem] bg-orange-500 text-white text-[9px] font-black rounded-full flex items-center justify-center px-0.5"
                >
          ...
          {/* ── Profile avatar ── */}
          <Link
            to={`/profile/${currentPet.id}`}
            aria-label={`${currentPet.name}'s profile`}
      ...
      <AnimatePresence>
        {searchOpen && (
          <motion.div
            ...
            className="md:hidden border-t border-[var(--pb-border-faint)] overflow-hidden"
          >
